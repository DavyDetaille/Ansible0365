...
$AntiPhishPolicy              = Get-AnsibleParam -obj $params -name "AntiPhishPolicy" -type "str"
$Enabled                      = Get-AnsibleParam -obj $params -name "Enabled" -type "bool"
$Priority                     = Get-AnsibleParam -obj $params -name "Priority" -type "int"
$Comments                     = Get-AnsibleParam -obj $params -name "Comments" -type "str"
$ExceptIfRecipientDomainIs    = Get-AnsibleParam -obj $params -name "ExceptIfRecipientDomainIs" -type "list"
$ExceptIfSentTo               = Get-AnsibleParam -obj $params -name "ExceptIfSentTo" -type "list"
$ExceptIfSentToMemberOf       = Get-AnsibleParam -obj $params -name "ExceptIfSentToMemberOf" -type "list"
$RecipientDomainIs            = Get-AnsibleParam -obj $params -name "RecipientDomainIs" -type "list"
$SentTo                       = Get-AnsibleParam -obj $params -name "SentTo" -type "list"
$SentToMemberOf               = Get-AnsibleParam -obj $params -name "SentToMemberOf" -type "list"
...
$inputData = @{
    ...
    Identity                     = $Identity
    Ensure                       = $Ensure
    AntiPhishPolicy              = $AntiPhishPolicy
    Enabled                      = $Enabled
    Priority                     = $Priority
    Comments                     = $Comments
...
function Actions($data){
    ...
    if($newData["Ensure"] -eq "Absent"){
        $result.msg = "This anti phish rule is revoved or don't exist"
    }else{
        $result.msg = "This anti phish rule is created or alredy exist"
    }
